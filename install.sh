...
execute_main_script() {
    ...
    # Check if the main script exists at the specified location
    MAIN_SCRIPT="$SCRIPT_DIR/src/main/ReconPro.py"
    ...
    if [ -f "$MAIN_SCRIPT" ]; then
        ...
        echo -e "${GREEN}[✓] Executing ReconPro from: $MAIN_SCRIPT${NC}\n"
        ...
        echo -e "${RED}[✗] Error: ReconPro.py not found in src/main directory!${NC}"
        echo -e "${YELLOW}[!] Please ensure ReconPro.py exists at: $MAIN_SCRIPT${NC}"
        
        # List contents to help debug
        echo -e "\n${CYAN}[*] Directory contents:${NC}"
        ls -la "$SCRIPT_DIR/src/" 2>/dev/null || echo "src directory not found"
        ls -la "$SCRIPT_DIR/src/main/" 2>/dev/null || echo "main directory not found"
...
main() {
    ...
    echo -e "\n${YELLOW}[?] Do you want to launch ReconPro now? (y/n)${NC} "
    read -n 1 -s launch_answer
    echo -e "\n"
    ...
    if [[ "$launch_answer" =~ ^[Yy]$ ]]; then
        ...
        echo -e "\n${GREEN}[✓] Installation complete! You can run ReconPro manually with:${NC}"
        echo -e "${YELLOW}cd $SCRIPT_DIR/src/main && python3 ReconPro.py${NC}"
        echo -e "${GREEN}Or activate the virtual environment first:${NC}"
        echo -e "${YELLOW}source $SCRIPT_DIR/reconpro_env/bin/activate${NC}"
        echo -e "${YELLOW}cd $SCRIPT_DIR/src/main && python3 ReconPro.py${NC}"
    fi
}
